...
const deletePet = (petId) => {
  if (confirm(`Are you sure to delete ${petId}?`)) {
    ...
    // delete in healthy pet arr
    if (
      petArr[deletePetIndex].vaccinated &&
      petArr[deletePetIndex].dewormed &&
      petArr[deletePetIndex].sterilized
    ) {
      const deleteHealthyPetIndex = healthyPet.findIndex(function (item) {
        return item.id === petId;
      });
      healthyPet.splice(deleteHealthyPetIndex, 1);
